...
namespace ControleEstoque.Web.Controllers.Cadastro
{
    ...
    public class CadPerfilController : BaseController
    {
        ...
        {
            ViewBag.ListaUsuario = Mapper.Map<List<UsuarioViewModel>>(UsuarioViewModel.GetAll());
            ViewBag.ListaTamPag = new SelectList(new int[] { _quantMaxLinhasPorPagina, 10, 15, 20 }, _quantMaxLinhasPorPagina);
            ViewBag.QuantMaxLinhasPorPagina = _quantMaxLinhasPorPagina;
            ViewBag.PaginaAtual = 1;

            var lista = Mapper.Map<List<PerfilViewModel>>(PerfilViewModel.RecuperarLista(ViewBag.PaginaAtual, _quantMaxLinhasPorPagina));
            var quant = PerfilViewModel.RecuperarQuantidade();
        ...
        {
            var lista = Mapper.Map<List<PerfilViewModel>>(PerfilViewModel.RecuperarLista(pagina, tamPag, filtro, ordem));
        ...
        {
            var ret = Mapper.Map<PerfilViewModel>(PerfilViewModel.RecuperarPeloId(id));
            return Json(ret);
        }
        ...
        {
            return Json(PerfilViewModel.ExcluirPeloId(id));
        }
        ...
        public JsonResult SalvarPerfil(PerfilViewModel model, List<int> idUsuarios)
        {
            ...
            else
            {
                ...
                try
                {
                    var vm = Mapper.Map<PerfilViewModel>(model);
                    var id = vm.Salvar();
                    if (id > 0)
                    {
                        idSalvo = id.ToString();
